...
const redisGroupChangeHistory = require('./redisGroupChangeHistory');

// 根选择不同数据表
function selectTable() {
    let Redis = Sequelize.define('umem_uredisgroup', {
        GroupId: {    // 唯一id 主键 string
            type: sequelize.STRING,
            primaryKey: true
        },
        Zone: sequelize.STRING,    // 可用区域
        GroupName: sequelize.STRING,   // redis组名称 string
        Name: sequelize.STRING,   // redis名称 string
        VirtualIP: sequelize.STRING,
        Port: sequelize.INTEGER,    // 端口
        MemorySize: sequelize.INTEGER,    // 内存大小
        Size: sequelize.INTEGER,
        UsedSize: sequelize.INTEGER,    // 已使用大小
        Version: sequelize.STRING,    // 版本
        Type: sequelize.STRING,    // 类型
        Protocol: sequelize.STRING,    // 数据库类型 redis\memcache
        HighAvailability: sequelize.STRING,
        AutoBackup: sequelize.STRING,
        BackupTime: sequelize.INTEGER,
        ConfigId: sequelize.STRING,
...
function creatData(arg) {
    ...
    let agrJson = JSON.stringify(arg);
    if (arg) {
        return Redis.create(arg).then((data) => {
            if (!data) {
                return {status: 500};
            }
            return redisGroupChangeHistory.createChangeInfo(arg, agrJson, "create");
        }).catch(function (err) {
            throw err;
        });
    }
...
function selectData(arg) {
    let Redis = selectTable();
    if (arg.hasOwnProperty('GroupId')) {
        return Redis.findOne({where: {GroupId: arg.GroupId, Survive: 1}}).then((data) => {
            if (!data) {
                return {status: 500};
            }
            return data;
        ...
        return Redis.findAll({where: {Protocol: "redis"}, limit: 10, offset: arg ? arg : 0}).then((data) => {
            if (!data) {
                return {status: 500};
            }
            return data;
...
function getHostInfoById(reg, arg) {
    return uclouldAuth.getUMemGroupInfo({Region: reg, argId: arg});
}
...
function redisInfoUpdate(arg) {
    ...
    if (arg.hasOwnProperty('GroupId')) {
        return Redis.update(arg, {where: {GroupId: arg.GroupId}}).then((data) => {
            if (!data) {
                return {status: 500};
            }
            return redisGroupChangeHistory.createChangeInfo(arg, "更改信息：" + argJson, "update");
        }).catch(function (err) {
            throw err;
        });
    } else {
...
function hostInfoDelAll(arg) {
    ...
    if (arg.hasOwnProperty('GroupId')) {
        ...
        return Redis.destroy({where: {GroupId: arg.GroupId}}).then((data) => {
            if (!data) {
                return {status: 500};
            }
            return redisGroupChangeHistory.createChangeInfo(arg, argJson, "delete");
        }).catch(function (err) {
            throw err;
        });
    } else {
...
function selectHistoryTable() {
    let Info = Sequelize.define("umem_redisgroup_change_history_info",{
        id : {    // 主键自增id
            type: sequelize.INTEGER,
            primaryKey: true
        },
        GroupId : sequelize.STRING,    // redis id
...
function cancleDelFun(createRes) {
    ...
    return Info.destroy({
        where: {
            GroupId: createRes.dataValues.GroupId,
            Action: "delete"
        }
    }).then((data) => {
        if (!data) {
...
module.exports = {
    ...
    redisInfoCopy: () => co(function *() {
        let hostArr = [];
        let bj2 = yield uclouldAuth.getUMemGroupInfo({Region: "cn-bj2"});
    ...
    redisInfoCount: () => {
        let Redis = selectTable();
        return Redis.count({where:{Protocol: "redis"}}).then((data) => {
            if (!data) {
                return {status: 500};
            }
            return data;
    ...
    redisInfoDelete: (arg) => co(function *() {
        let delHost = yield selectData({GroupId: arg});
    ...
    hostDropInfo: (arg) => {
        let Info = selectHistoryTable();
        if (arg) {
            return Info.findAll({where: {GroupId: arg, Action: "delete"}}).then((data) => {
                if (!data) {
                    return {status: 500};
                }
                return data;
